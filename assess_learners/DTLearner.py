"""
A simple wrapper for Decision Tree
"""
...
class DTLearner(object):
    ...
    def buildTree(self, xdata, ydata):
        ...
        for j in range(xdata.shape[1]):
            correlation = np.abs(np.corrcoef(x = xdata[:,j], y = ydata))
            correlationList.append(correlation[0][1])
        splitIndex = np.argmax(correlationList)
